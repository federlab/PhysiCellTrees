...
foreach(analysistype = c("2D_neut_bdg", "2D_sel_bdg_highermu", "3D_neut" ), .combine = "rbind")%do%{
...
#can uncomment the above to just get a single example per d
    ...
    foreach(fi = list.files(fullhullpath)[inds], .combine = "rbind")%do%{
        ...
        if(grepl("2D_sel_bdg", analysistype)){
            ...
            blues <- blueramp(4)
            maroons <- maroonramp(4)

            tumor_cols <- c(blues[1:3], maroons[1:3])
            names(tumor_cols) <- c("TRUE 0", "TRUE 1", "TRUE 2", "FALSE 0", "FALSE 1", "FALSE 2")
            toPlot <- (datToPlot %>% mutate(numDriv = log(growth/min(hullinf$growth), base = 1.1)) %>% arrange(ID) %>%
